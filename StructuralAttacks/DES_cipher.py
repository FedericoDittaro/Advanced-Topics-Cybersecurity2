##### Code taken from https://medium.com/@ziaullahrajpoot/data-encryption-standard-des-dc8610aafdb3
...
def encryption(user_input):
    ...
    for round_num in range(16):
        ...
        #if round_num==15:
        #    print("Expanded result: ", expanded_result)
...
def fault_encryption(user_input, fault_bit):
    binary_rep_of_input = str_to_bin(user_input)
    round_keys = generate_round_keys()

    ip_result_str = ip_on_binary_rep(binary_rep_of_input)

    lpt = ip_result_str[:32]  # Left half (32 bits)
    rpt = ip_result_str[32:]  # Right half (32 bits)
    ...
    for round_num in range(16):
        ...
        # XOR with LPT
        new_rpt = [
            str(int(lpt[i]) ^ int(p_box_result[i])) for i in range(32)
        ]

        # Inject fault during the 15th round
        if round_num == 14:
            new_rpt[fault_bit] = '1' if new_rpt[fault_bit] == '0' else '0'
            print(f"Fault injected in the right half at round 15, bit {fault_bit}")

        # Update LPT and RPT for the next round
        lpt, rpt = rpt, ''.join(new_rpt)
...
def reverse_cipher(c):
    c_bin = ip_on_binary_rep(str_to_bin(c))
    l16 = c_bin[32:]
    r16 = c_bin[:32]
    r15 = l16

    p_box_inverse = [0] * len(p_box_table)
    for i, p in enumerate(p_box_table):
        p_box_inverse[p - 1] = i + 1

    unpermuted = ''.join([r16[i-1] for i in p_box_inverse])
    output = unpermuted[4:8]

    expanded_result = [r15[i - 1] for i in e_box_table]
    expanded_result_str = ''.join(expanded_result)

    input = expanded_result_str[6:12]

    return input, output


def apply_substitution(c_in):
    substituted_box = ''

    row_bits = int(c_in[0] + c_in[-1], 2)
    col_bits = int(c_in[1:-1], 2)

    s_box_value = s_boxes[1][row_bits][col_bits]

    # Convert the S-box value to a 4-bit binary string and append to the result
    substituted_box += format(s_box_value, '04b')

    return substituted_box
...
if __name__=="__main__":
    ...
    c = encryption(plaintext)
    c_in, c_out = reverse_cipher(c)

    fault_c = fault_encryption(plaintext, fault_bit)
    c_fault_in, c_fault_out = reverse_cipher(fault_c)


    for k16 in range(64):
        round_key = format(k16, '06b')

        c_xor_result = ''
        c_fault_xor_result = ''
        for i in range(6):
            c_xor_result += str(int(c_in[i]) ^ int(round_key[i]))
            c_fault_xor_result += str(int(c_fault_in[i]) ^ int(round_key[i]))

        c_s_box = apply_substitution(c_xor_result)
        c_fault_s_box = apply_substitution(c_fault_xor_result)

        right_part = ''.join([str(int(c_s_box[i]) ^ int(c_fault_s_box[i])) for i in range(4)])

        left_part = ''.join([str(int(c_out[i]) ^ int(c_fault_out[i])) for i in range(4)])
