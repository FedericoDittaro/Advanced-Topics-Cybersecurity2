...
class TRS_Reader(object):
    Number_of_Traces=0
    index=0                     # Trace index
    number_of_samples=0         # Number of samples per trace
    isint=True                  # Indicates if the trace data is stored as integers or floats
    cryptolen=0                 # Length of the cryptographic data (plaintext and ciphertext) in bytes
    traces = None
    plaintext= None
    ciphertext= None
    pos=0                       # Starting position of data section in the file

    def __init__(self,filename):
        self.f = open(filename, 'rb+')    # opens the file in binary mode for reading and writing
    def __del__(self):
        self.f.close()  # Ensures the file is closed when the object is deleted

    def read_header(self):     # Parses the header of the file to extract metadata
        self.index = 0
        # Read Header Info
        # Tag   Len Discription
        ...
        while(True):
            ...
            if tag==b'\x43':     # 0x43 is the trace format (integer or float) 
                ...
                if self.f.read(len)==b'\x02':
                    ...
                    self.isint=False
                continue
            if tag == b'\x44':    # 0x44 is the cryptographic data length
                len = int.from_bytes(self.f.read(1), byteorder='little')
                self.cryptolen=int.from_bytes(self.f.read(len), byteorder='little')
                continue
            if tag == b'\x5F':    # 0x45 is the data section start position
                len = int.from_bytes(self.f.read(1), byteorder='little')
                self.pos = self.f.tell()   # Stops reading the header when it reaches the start of the data
                break
            len = int.from_bytes(self.f.read(1), byteorder='little')
            self.f.read(len)

    # Reads trace data and optionally a subset of it
    def read_traces(self,N=0,startp=0,endp=0):   
        if(N==0):
            N=self.Number_of_Traces
        if(endp==0):
            endp=self.number_of_samples
        # Populates the traces, plaintext (as byte), and ciphertext (as byte) arrays
        ...
        self.index=0             # Resets the file pointer and index to allow repeated reads

    # Extracts data for a single sample (no) across N traces
    # Returns a 1D array with the sample data
    ...
    # Reads only the plaintext and ciphertext data for N traces
    # Returns the plaintext and ciphertext arrays
